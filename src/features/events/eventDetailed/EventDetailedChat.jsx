...
export default function EventDetailedChat({ eventId }) {
  ...
  return (
    <>
      ...
      {authenticated && (
        <Segment attached>
          <EventDetailedChatForm eventId={eventId} parentId={0} setShowReplyForm={setShowReplyForm} />
          <Comment.Group>
            {createDataTree(comments).map(comment => (
              <Comment key={comment.id}>
                ...
                <Comment.Content>
                  ...
                  <Comment.Actions>
                    <Comment.Action onClick={() => setShowReplyForm({ open: true, commentId: comment.id })}>
                      Reply
                    </Comment.Action>
